...
class TC_PushPullTool_Model < TestUp::TestCase
  def test_test_plane
    profile = Parametric::Geom::Polygon.new([[1,1], [3,3,1], [1,5]].map { |params| Geom::Point3d.new params })
    model = Parametric::Tools::PushPullTool::Model.new profile
    subject = model.test_plane
    assert_nil subject.intersect_line([profile.first, model.profile.plane.normal])
    assert profile.center.on_plane? subject
  end
end

class TC_PushPullTool < TestUp::TestCase
...
class TC_PushPullTool_Stage < TestUp::TestCase
  def test_done
    ...
    subject = Parametric::Tools::PushPullTool.as_sage(profile) { |params| done_flag.raise params.transform_values(&:class) }
    subject.activate
