...
class TC_XYZTool_Model < TestUp::TestCase
  
  attr_reader :subject

  def setup
    @subject ||= Parametric::Tools::XYZTool::Model.new(@trn || IDENTITY)
  end
...
class TC_XYZTool < TestUp::TestCase
...
class TC_XYZTool_Stage < TestUp::TestCase
  attr_reader :subject

  def setup
    expected_params = { view: Sketchup::View, origin: Geom::Point3d, vectors: Array,  }
    @done_flag = Minitest::Mock.new.expect(:raise, nil, [expected_params])
    flag = @done_flag
    @subject = Parametric::Tools::XYZTool.as_stage { |params| flag.raise params.transform_values(&:class) }
    @subject.activate
    @subject
  end
