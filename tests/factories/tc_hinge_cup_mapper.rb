...
class TC_HingeCupMapper < TestUp::TestCase
	def test_created_map
		cup_map = Parametric::HingeCupDrilling::Mapper.new.build(bore: 4.mm, map: '52x5.5')
	...
	def test_invalid_map_raises
		e = assert_raises { Parametric::HingeCupDrilling::Mapper.new(bore: 4, map: 'fake') }
		assert_match 'fake', e.message
	end

	def test_with_regular_params
		assert_instance_of Parametric::Maps::HingeCup,
			Parametric::HingeCupDrilling::Mapper.new.build(
				bore: 4.mm,
