...
class Parametric::Geom::Flatten
	...
	def profile_sides
		@faces ||= [
			Parametric::Geom::Polygon.new(@profile),
			Parametric::Geom::Polygon.new(@profile.map { |corner| corner + @vector }.rotate!.reverse!)
		]
	end

	def edge_sides
		edges = @profile.map { |corner| [corner, corner + @vector] }
		edges.zip(edges.rotate).map { |(p1, p2), (p4, p3)| Parametric::Geom::Polygon.new p1, p2, p3, p4 }
	end
