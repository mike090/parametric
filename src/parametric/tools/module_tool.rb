		...
		DEFAULT_PANEL_PARAMS = {
			...
			def run
				using :box_tool  do |box_tool_result|
					...
					stage = ModuleBuilder.new box_params
					tool = self
			...
			class ModuleBuilder
				...
				# 	@sketchup_model.start_operation 'Panel'
				# 	faces = preview.sides.map { |side| @sketchup_model.entities.add_face side }
				# 	panel = @sketchup_model.entities.add_group faces
				# 	panel.name = 'Panel'
				# 	@sketchup_model.commit_operation
				# 	@focused.panel = panel
				end
			end	
			...
			class Model
				...
				def profile_bounds(external = false)
					...
					side = @focused.side
					return Parametric::Geom::Polygon.new side if external
