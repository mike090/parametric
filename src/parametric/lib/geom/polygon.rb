...
class Parametric::Geom::Polygon < Array
	...
	def initialize(*vertices)
		...
		raise TypeError, "wrong argument type #{vertices.class} (expected Array)" unless vertices.is_a?(Array)
		raise ArgumentError, 'At least three points needed to initialize polygon' if vertices.length < 3

		plane = Parametric::Geom::Plane.new vertices
		raise ArgumentError, 'the vertices do not lie in the same plane' unless plane
		
		super vertices.map { |pos| Geom::Point3d.new pos }
	end
	...
	def reverse
		super.rotate(-1)
		# Parametric::Geom::Polygon.new(super().rotate(-1))
	end
